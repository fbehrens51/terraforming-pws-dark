...
aws ec2 register-image --name $AMI_NAME --virtualization-type hvm --root-device-name "/dev/xvda" --block-device-mappings "[{\"DeviceName\": \"/dev/xvda\",\"Ebs\":{\"SnapshotId\":\"$SNAPSHOTID\"}}]"