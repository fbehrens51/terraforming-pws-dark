...
resource "aws_security_group_rule" "ingress_from_cp" {
...
resource "aws_security_group_rule" "ingress_from_elb" {
  count                    = length(var.elb_security_group_ids)
  description              = "Allow all protocols/ports from the ELB"
  security_group_id        = aws_security_group.vms_security_group[0].id
  type                     = "ingress"
  source_security_group_id = var.elb_security_group_ids[count.index]
  protocol                 = "-1"
  from_port                = 0
  to_port                  = 0
}
...
resource "aws_security_group_rule" "ingress_from_self" {
  description       = "Allow all ingress from ${aws_security_group.vms_security_group[0].name}"
  security_group_id = aws_security_group.vms_security_group[0].id
  type              = "ingress"
  self              = true
  protocol          = "-1"
