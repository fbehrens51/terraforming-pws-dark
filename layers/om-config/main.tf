...
module "om_config" {
  ...
  env_name                    = join(", ", [var.global_vars.env_name, "iaas:${data.terraform_remote_state.paperwork.outputs.foundation_name}_tas", "foundation_name:${data.terraform_remote_state.paperwork.outputs.foundation_name}"])
