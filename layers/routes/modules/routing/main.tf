...
resource "aws_route" "route_to_add_to_accepter" {
  ...
  route_table_id            = element(tolist(data.aws_route_tables.accepter_route_table.ids), count.index)
...
resource "aws_route" "route_to_add_to_requestor" {
  count                     = length(var.availability_zones) + 1
  route_table_id            = element(tolist(data.aws_route_tables.requester_route_table.ids), count.index)
