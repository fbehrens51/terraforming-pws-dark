...
write_files:
  ...
  - path: /usr/local/bin/install-tar-tifact.sh
    permissions: '0755'
    owner: root:root
    content: |
      #!/usr/bin/env bash
  ...
  - path: /usr/local/bin/install-tanzu.sh
    ...
    content: |
      ...
      install-tar-tifact.sh tkg-artifacts tkg-images
  - path: /usr/local/bin/post-init.sh
    permissions: '0755'
    owner: root:root
    content: |
      echo "post-init was run!"
  - path: /etc/profile.d/tws_env.sh
    ...
    content: |
      ...
      export TKG_CUSTOM_IMAGE_REPOSITORY="$(hostname -I | awk '{print $1}')"
      export REGISTRY_CACHE=$HOME/docker

runcmd:
  - |
    # echo "Installing up system tools and utilities"
    yum install jq git python-pip python3 openldap-clients docker -y
...
//tar -xvf "$local_repo/$artifact/*.tar.gz" -C "$artifact_dir"
//      rm -r "$local_repo"
//install-tar-tifact.sh tkg-artifacts
#
...
// /usr/local/bin/pwsd release download --components "tkg-artifacts" --output "workspace"
// cd $HOME/workspace/tkg-artifacts && tar -xvf tkg-artifacts-*
// rm tkg-1.4.0-images-*.tar.gz
// /usr/local/bin/pwsd release download --components "tkg-images" --output "workspace"
// cd $HOME/workspace/tkg-images && tar -xvf tkg-1.4.0-images-1.0.1.tar.gz
//TODO: Mount drive with images or where images can go
...
resource "null_resource" "tkgjb_status" {
  ...
  provisioner "local-exec" {
    ...
    poll_tags="aws ec2 describe-tags --filters Name=resource-id,Values=${module.tkgjb.instance_ids[count.index]} Name=key,Values=$completed_tag --output text --query Tags[*].Value"
    ...
    if cloud_init_message="$( aws ec2 describe-tags --filters Name=resource-id,Values=${module.tkgjb.instance_ids[count.index]} Name=key,Values=cloud_init_output --output text --query Tags[*].Value )"; then
...
output "ssh_host_ips" {
  value = zipmap(flatten(module.tkgjb.ssh_host_names), flatten(module.tkgjb.private_ips))
}
