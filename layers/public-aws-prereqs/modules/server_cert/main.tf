...
output "private_key_pem" {
  value     = "${element(concat(tls_private_key.server_private_key.*.private_key_pem, list("")), 0)}"
  sensitive = true
}

output "cert_pem" {
  value = "${element(concat(tls_locally_signed_cert.server_cert.*.cert_pem, list("")), 0)}"
}
